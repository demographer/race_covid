## Estimation and figures for indirect standardization of COVID-19 deaths by age and place (differenced)
...
## Deaths by age (Di)
Di.df = read.csv("../data/deaths_by_age_Di_diff.csv", header = T, comment.char = "#")
Di = Di.df$nDx
...
## Deaths by county (Dj)
cdc_county_filename = "../data/provisional_COVID-19_death_counts_by_county_june_13.csv"
Dj = get_Dj(state_name = state.name, cdc_county_filename) ## state.name is all 50 states, but not DC

# correct spelling error (this is new here)
names(Dj)[names(Dj) == "La Porte County, Indiana"] <- "LaPorte County, Indiana"



## Deaths by race for states and nation (DJk)
perc_DJk = fread("../data/clean_cdc_race_diff.csv")
...
my_counties = names(Dj) ## now it uses "LaPorte"
...
## Dk for nation
cleaned_cdc_race_filename = "../data/clean_cdc_race_diff.csv"
...
## try with CI
if(produce_figures)
    pdf("../figures/state_diff.pdf", height = 12, width = 8)
par(mfrow = c(2,1))
...
{
    dev.off()
    system("open ../figures/state_diff.pdf")
}
...
if(produce_figures)
    pdf("../figures/usa_all_diff.pdf", width = 12, height = 9)
par(mfrow = c(1,1))
...
if(produce_figures)
{    dev.off()
    system("open ../figures/usa_all_diff.pdf")
}



## write tables to folder
# write.csv(Di.df, file = "../data/Di.df_diff.csv")
# write.csv(perc_DJk, file = "../data/perc_DJk_diff.csv")
# write.csv(states_with_race_reports.dt, file = "../data/states_diff.csv")
